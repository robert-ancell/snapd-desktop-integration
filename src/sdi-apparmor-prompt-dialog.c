...
static void respond(SdiApparmorPromptDialog *self, gboolean allow,
  ...
  g_autoptr(GVariantBuilder) extra_constraints =
      g_variant_builder_new(G_VARIANT_TYPE("a{ss}"));
  if (allow_directory) {
    g_variant_builder_add(extra_constraints, "{ss}", "allow-directory", "yes");
  }
  if (always_prompt) {
    g_variant_builder_add(extra_constraints, "{ss}", "always-prompt", "yes");
  }
  prompt_agent_complete_prompt(self->prompt_agent, self->invocation, allow,
                               g_variant_builder_end(extra_constraints));
...
sdi_apparmor_prompt_dialog_new(PromptAgent *prompt_agent,
  ...
  self->prompt_agent = g_object_ref(prompt_agent);
