...
static SdiRefreshDialog *
lookup_application(SdiRefreshMonitor *self,
                                       const char *app_name) {
...
static gboolean dbus_handle_application_is_being_refreshed(SdiRefreshMonitor *self,
							   GDBusMethodInvocation *invocation,
    gchar *snap_name, gchar *lockFilePath, GVariant *extra_params) {
  SdiRefreshDialog *dialog = get_dialog(self, snap_name);

  //handle_application_is_being_refreshed(snap_name, lockFilePath, extra_params,
  //                                      data);
  snap_desktop_integration_complete_application_is_being_refreshed(self->skeleton,
                                                                   invocation);
  return TRUE;
}

static gboolean dbus_handle_application_refresh_completed(SdiRefreshMonitor *self, GDBusMethodInvocation *invocation,
    gchar *snap_name, GVariant *extra_params) {
  SdiRefreshDialog *dialog = get_dialog(self, snap_name);

  //handle_application_refresh_completed(snap_name, extra_params, data);
  snap_desktop_integration_complete_application_refresh_completed(self->skeleton,
                                                                  invocation);
  return TRUE;
}

static gboolean dbus_handle_set_pulsed_progress(SdiRefreshMonitor *self, GDBusMethodInvocation *invocation,
    gchar *snap_name, gchar *bar_text, GVariant *extra_params) {
  SdiRefreshDialog *dialog = get_dialog(self, snap_name);
...
static gboolean dbus_handle_set_percentage_progress(SdiRefreshMonitor *self,
						    GDBusMethodInvocation *invocation,
  ...
  //                               data);
  snap_desktop_integration_complete_application_refresh_percentage(self->skeleton,
                                                                   invocation);
  return TRUE;
}
...
gboolean sdi_refresh_monitor_start(SdiRefreshMonitor *self,
                                   ...
                                   GError **error) {
  g_signal_connect_swapped(self->skeleton, "handle_application_is_being_refreshed",
                   G_CALLBACK(dbus_handle_application_is_being_refreshed),
                   self);
  g_signal_connect_swapped(self->skeleton, "handle_application_refresh_completed",
  ...
  g_signal_connect_swapped(self->skeleton, "handle_application_refresh_percentage",
